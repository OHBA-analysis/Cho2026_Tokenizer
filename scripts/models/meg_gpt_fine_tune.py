...
- Pre-trained subject embeddings: the model can load and incorporate subject
  embeddings if enabled in the config.
- Task-specific prediction head: the final head is configured for four-way
  classification corresponding to the WH task conditions.
"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pretrained_layer is None:
            ...
            self.extra_embedding_layers = pretrained_layer.extra_embedding_layers
            self.extra_embedding_output_layers = (
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        for label, layer, output_layer in zip(
            ...
            embeddings += output_layer(
                layer(label, training=training_1), training=training_1
            )
...
class DecoderLayer(tf.keras.layers.Layer):
    """
    Layer for the decoder of the MEGGPT_FT model.
...
class MEGGPT_FT(BaseModel):
    """
    MEG-GPT model for fine-tuning on the downstream task dataset.
    ...
    def _build_model(self) -> tf.keras.Model:
        ...
        return tf.keras.Model(
            inputs=[data] + extra_labels + [task_label],
            outputs=[loss, y_pred],
            name="meg_gpt_fine_tune",
        )
